...
def submit(request):
    if request.method == "POST":
        ...
        if request.POST.get('link'):
            ...
            listtable.link = "http://www.neo-furniture.com/media/catalog/product/cache/10/image/9df78eab33525d08d6e5fb8d27136e95/b/8/b805.jpg"
        listtable.time = dt
        listtable.save()
        all = Alllisting()
        items = Listing.objects.all()
...
def yourwinnings(request):
    if request.user.username:
        ...
        try:
            ...
            wcount=None
        return render(request,'auctions/yourwinnings.html',{
            "items":items,
            "wcount":wcount,
            "wonitems":wonitems
        })
